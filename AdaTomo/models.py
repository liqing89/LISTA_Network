...
import pandas as pd
path = os.getcwd()
base_path = os.path.join(path,'middle_result','Ada-LISTA')
# 创建空行字符串
empty_row = ''
##################################################
...
class Adaptive_ISTA(nn.Module):
    ...
    def forward(self, y, D ,epoch):
        ...
        if y.is_cuda:
            x = x.cuda()
        # file_name = os.path.join(base_path,f'Ada_LISTA_epoch{epoch}_output.csv')
        # data =[]
...
class LISTA(nn.Module):
    def __init__(self, n, m, D=None, T=10, lambd=0.5):
        ...
        # Initialization
        if D is not None:
            self.A.weight.data = torch.from_numpy(D.transpose())
            self.B.weight.data = torch.from_numpy(D.transpose()@D)
            L = round(float(eigvalsh(D.T @ D, eigvals=(m - 1, m - 1)))) 
            self.etas.data *= 1 / L
            self.gammas.data *= 1 / L
        self.reinit_num = 0  # Number of re-initializations
